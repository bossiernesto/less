...
case class EncodeResult(text: String = "") {
	def ~(r: EncodeResult) = copy(text ++ r.text)
}

object Encode extends Encode { def _syntax = Syntax }
trait Encode {
	implicit def String_To_EncodeResult(s: String) = EncodeResult(s)

	def apply(sentences: Sentence*)(implicit prefs: PreferenceFixture) = {
		val r = (EncodeResult() /: sentences){ (r, s) => encode(s)(prefs) ~ lineSeparator(s) }
		if (r.text.endsWith("\n")) r.copy(r.text.init) else r
	}

	def _syntax: Syntax
	def syntax(key: Symbol) = _syntax(key).replace("""\""", """""")

	protected def encode(sentence: Sentence, level: Int = 0)(implicit prefs: PreferenceFixture): EncodeResult = {
		...
		val content: EncodeResult = sentence match {
			case R(id) => EncodeResult(id.value.name)

			case O(id, body) =>
				syntax('objectKeyword) ~ " " ~ id.value.name ~ s("After object name") ~ syntax('codeOpen) ~ "\n" ~
					(EncodeResult() /: body){ (r, s) => encode(s, level + 1) ~ lineSeparator(s) } ~
					tabulation ~ syntax('codeClose)

			case M(id, args, body) =>
				syntax('defKeyword) ~ " " ~ id.value.name ~ s("After method name") ~
					syntax('parensOpen) ~ s("Before each method argument", args.nonEmpty) ~ args.map(_.value.name).mkString(s("Before each method argument") ++ syntax('sentenceSep) ++ s("After each method argument")) ~ syntax('parensClose) ~ s("After method arguments") ~
					syntax('codeOpen) ~ "\n" ~
					(EncodeResult() /: body){ (r, s) => encode(s, level + 1) ~ lineSeparator(s) } ~
					tabulation ~ syntax('codeClose)

			case A(values) => syntax('arrayOpen) ~ values.map(encode(_)).mkString(s("Before each array argument") ++ syntax('sentenceSep) ++ s("After each array argument")) ~ syntax('arrayClose)
			...
			case Send(targetR, messageName, arguments) => encode(targetR) ~ syntax('access) ~ messageName.value.name ~ s("After message name") ~
				syntax('argOpen) ~ s("Before each message argument", arguments.nonEmpty) ~ arguments.map(encode(_)).mkString(s("Before each message argument") ++ syntax('sentenceSep) ++ s("After each message argument")) ~ syntax('argClose)
